"""Evaluate v11 iter-prior candidates with Full ESMFold2."""
...
def evaluate(snaps, out_path, init_seq, include_init=True):
    print(f"=== Evaluating {len(snaps)} v11 candidates with Full model ===\n",
          flush=True)
    ...
    for snap in snaps:
        eval_list.append((f"v11 step {snap['step']}", snap["full_seq"]))
...
if __name__ == "__main__":
    ...
    p.add_argument("--snaps", default="/tmp/b5_v11_iter_prior_snaps.json")
    p.add_argument("--out", default="/tmp/b5_v11_eval.json")
    args = p.parse_args()
