"""Evaluate v10a iter candidates with Full ESMFold2."""
...
def evaluate(snaps, out_path, init_seq, include_init=True):
    print(f"=== Evaluating {len(snaps)} v10a candidates with Full model ===\n",
          flush=True)
    ...
    for snap in snaps:
        eval_list.append((f"v10a step {snap['step']}", snap["full_seq"]))
...
if __name__ == "__main__":
    ...
    p.add_argument("--snaps", default="/tmp/b5_v10a_iter_snaps.json")
    p.add_argument("--out", default="/tmp/b5_v10a_eval.json")
    args = p.parse_args()
