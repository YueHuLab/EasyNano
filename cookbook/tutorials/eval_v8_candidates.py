"""Evaluate v8 3-CDR finetune candidates (Full-predicted prior) with Full ESMFold2."""
...
def evaluate(snaps, out_path, init_seq, include_init=True):
    print(f"=== Evaluating {len(snaps)} v8 candidates with Full model ===\n", flush=True)
    ...
    eval_list = []
    if include_init:
        eval_list.append(("v2 step050 (init)", init_seq))
    for snap in snaps:
        eval_list.append((f"v8 step {snap['step']}", snap["full_seq"]))
...
if __name__ == "__main__":
    ...
    p.add_argument("--snaps", default="/tmp/b5_v8_fullprior_snaps.json")
    p.add_argument("--out", default="/tmp/b5_v8_eval.json")
    args = p.parse_args()

    snaps = load_v8_snaps(args.snaps)
    if not snaps:
        print("No snapshots found!", flush=True)
        sys.exit(1)
    init_seq = snaps[0]["full_seq"]  # use first snap's init (matches design script)
    # If snaps are sorted by step, the first one (step 0?) won't have full_seq
    # The init_full_seq is in the json wrapper
