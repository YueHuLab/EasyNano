"""Evaluate v10c iter candidates with Full ESMFold2."""
...
def evaluate(snaps, out_path, init_seq, include_init=True):
    print(f"=== Evaluating {len(snaps)} v10c candidates with Full model ===\n",
          flush=True)
    ...
    eval_list = []
    if include_init:
        eval_list.append(("v10b step 80 (init)", init_seq))
    for snap in snaps:
        eval_list.append((f"v10c step {snap['step']}", snap["full_seq"]))
...
if __name__ == "__main__":
    ...
    p.add_argument("--snaps", default="/tmp/b5_v10c_official_plm_snaps.json")
    p.add_argument("--out", default="/tmp/b5_v10c_eval.json")
    args = p.parse_args()
